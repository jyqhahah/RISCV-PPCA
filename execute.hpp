...
class execute{
public:
    void JUMP_inst(IF_ID &if_id, ID_EX &id_ex, EX_MEM &ex_mem){
        switch(id_ex.inst.inst){
            ...
            case BEQ: {
                bool willJump = id_ex.reg_rs1 == id_ex.reg_rs2;
                if (willJump){
                    pc = id_ex.NPC + id_ex.inst.imm;
                    if_id.isNext = 0;
                }
    ...
    void exe(IF_ID &if_id, ID_EX &id_ex, EX_MEM &ex_mem){
